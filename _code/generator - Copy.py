...
def generate_text(patterns, text_size, anomaly_ratio, vocabulary, patterns_p=None):
    p_i = np.random.choice(a=range(len(patterns)), size=text_size, p=patterns_p)
...
def generate_tests(patterns, vocabulary, n, text_size, anomaly_ratio):
    tests = dict()
    for i in range(n):
        text_test , marks_test = generate_text(patterns, text_size=text_size , anomaly_ratio=anomaly_ratio, vocabulary=vocabulary)
